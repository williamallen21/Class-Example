class vehicle { //setting up instant class
    int passangerCap;
    int numDoors;
    int fuelCap;
    int mpg;
...
vehicle (int pc, int nd, int fc, int fe, int ts, double ac, int bat) { //making the constructor for the vehicles
    passangerCap = pc;
    numDoors = nd;
    fuelCap = fc;
    mpg = fe;
    ...
    double laToLv (double mpg) { // calculates how many tanks it would take for the car to go from Los Angeles to Las Vegas
        double gallons = (double)270.2/mpg;
        gallons = Math.round(gallons);
        return gallons;
    }
    double laTosd (double mpg) { // calculates how many tanks it would take for the car to go from Los Angeles to San Diego
        double gallons = (double)120.3/mpg;
        gallons = Math.round(gallons);
        return gallons;
}
  double laToaz (double mpg) { // calculates how many tanks it would take for the car to go from Los Angeles to Arizona
...
class Main {
  ...
  vehicle teslaS = new vehicle (5, 4, 315, 90, 155, 2.5, 100); 
  vehicle tesla3 = new vehicle (5, 4, 289, 28, 140, 3.3, 75); 
  vehicle TeslaX = new vehicle (7, 4, 298, 88, 130, 2.9, 100); 
  /* 
...
*/     
    ...
    System.out.println("You can go from Los Angeles to San Diego using approximatly " + teslaS.laToaz(teslaS.mpg) + " full tanks.");
    ...
    System.out.println("You can go from Los Angeles to San Diego using approximatly " + tesla3.laToaz(tesla3.mpg) + " full tanks.");
    ...
    System.out.println("You can go from Los Angeles to San Diego using approximatly " + teslaX.laToaz(teslaX.mpg) + " full tanks.");
}
  }
